...
function buildDashboardHtml(artifacts) {
...
export async function writeRunArtifacts(outputDir, findings, telemetry) {
    await mkdir(outputDir, { recursive: true });
    const artifacts = { findings, telemetry };
    await writeFile(path.join(outputDir, "run.json"), JSON.stringify(artifacts, null, 2), "utf-8");
    const markdown = [
        "# Bug Hunter Report",
        "",
        `Total findings: ${findings.length}`,
        ...
        "## Route timings",
        "",
        ...telemetry.map((t) => `- ${t.route} (${t.viewport}): ${t.durationMs}ms | console=${t.consoleErrors.length} | network=${t.networkFailures.length + t.httpFailures.length}`)
    ].join("\n");
